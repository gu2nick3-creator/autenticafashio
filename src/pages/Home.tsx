...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      const lastIndex = heroProducts.length - 1;
  ...
  }, [heroCarouselApi, heroProducts.length]);
          ...
          <CarouselContent className="ml-0 h-full">
            {heroProducts.map((product) => (
              <CarouselItem key={product.id} className="pl-0 basis-full h-full">
                <Link to={`/produto/${product.slug}`} className="block h-full w-full" aria-label={`Abrir produto ${product.name}`}>
                  <div className="relative h-full min-h-[540px] md:min-h-[720px] w-full overflow-hidden bg-[#f4efe8]">
                    <img src={product.images[0] || heroImage} alt={product.name} className="h-full w-full object-cover object-center" />
        ...
        <div className="relative z-10 container flex min-h-[540px] md:min-h-[720px] items-center justify-center py-16 md:justify-start md:py-24">
          <ScrollReveal>
            <div className="mx-auto flex max-w-[330px] flex-col items-center text-center text-white md:mx-0 md:max-w-2xl md:items-start md:text-left">
              <p className="mb-4 text-[11px] font-medium uppercase tracking-[0.45em] text-white/80 md:text-xs">Nova coleção</p>
